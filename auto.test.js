...
const testEmails = ["applicant@test.com", "reviewer@test.com", "bob@test.com"];
...
describe("Submission API", () => {
  ...
  // ----------------------
  beforeAll(async () => {
    const applicantRes = await request(app).post("/api/v1/auth/login").send({
      email: "applicant@test.com",
      password: "pass123",
    });
    ...
    const reviewerRes = await request(app).post("/api/v1/auth/login").send({
      email: "reviewer@test.com",
      password: "pass123",
    });
